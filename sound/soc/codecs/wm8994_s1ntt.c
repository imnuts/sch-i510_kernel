...
// Ear MIC
#define TUNING_RECORD_SUB_INPUTMIX_VOL	0x08 //0x15		// 1Ah
...
void audio_ctrl_mic_bias_gpio(int enable)
{
	...
	if(enable)
	{
		...
		DEBUG_LOG("Main/Ear MIC Bias enable = [%d]", enable);
	}
	else
	{
		set_recording_status(0);
		gpio_set_value(GPIO_MICBIAS_EN, 0);
		DEBUG_LOG("Main MIC Bias enable = [%d]", enable);
		headset_state = get_headset_status();
		
		if( headset_state == 0 ||headset_state == 0x1 << 5 || headset_state == 0x01 << 1 )	// no headset or TV out or  3pol headset
		{
			gpio_set_value(GPIO_SUB_MICBIAS_EN, 0);
			DEBUG_LOG("Ear MIC Bias enable = [%d]", enable);
		}
	}
}
