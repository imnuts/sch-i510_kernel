...
//extern short int get_headset_status(void);		// For ear-jack control(MIC-Bias)//sayanta ..not present in 32 kernel now
//extern void set_recording_status(int value);	// For preventing MIC Bias off on using MIC.//Fucntion not present in SLSI platform
...
void audio_ctrl_sleep_gpio(int enable)
{
  ...
  #if 0 //(defined CONFIG_JUPITER_VER_B5)  || (defined CONFIG_ARIES_VER_B0)
...
int audio_power(int en)
{
	...
	if(en)
	{
		...
		msleep(10);	// Wait for warming up.//commenting sleep as not required in SLSI platform

		// Turn on master clock.
			__raw_writel(__raw_readl(S5P_OTHERS) | (3<<8) , S5P_OTHERS);//sayanta ..check this is executing or not 
	...
	else
	{
		...
		msleep(125);	// Wait to turn off codec entirely.////commenting sleep as not required in SLSI platform
...
//		__raw_writel(__raw_readl(S5P_CLK_OUT) & (0xFFFFFFFE) , S5P_CLK_OUT);
	...
	if(en)
	{
		...
		// Turn on master clock.
#if (defined CONFIG_JUPITER_VER_B5)  || (defined CONFIG_ARIES_VER_B0) || (defined CONFIG_ARIES_VER_B1) || (defined CONFIG_ARIES_VER_B2) || (defined CONFIG_ARIES_VER_B3)
	...
	else
	{
		...
		val &= ~(0x5 << 20);
#endif	//end of CONFIG_SND_UNIVERSAL_WM8994_MASTER
#elif (defined CONFIG_ARIES_VER_B0) || (defined CONFIG_ARIES_VER_B1) || (defined CONFIG_ARIES_VER_B2) || (defined CONFIG_ARIES_VER_B3)
		if(!hw_version_check()) //evt0 aries00 board
                gpio_set_value(GPIO_CODEC_XTAL_EN, 0);
#if (defined CONFIG_SND_UNIVERSAL_WM8994_MASTER)
		else{
...
void audio_ctrl_mic_bias_gpio(int enable)
{
	...
	if(enable)
	{
		//set_recording_status(1);//commented as this function is not present in SLSI platform
		gpio_set_value(GPIO_MICBIAS_EN, 1);
	}
	else
	{
		//set_recording_status(0);//commented as this function is not present in SLSI platform
		//headset_state = get_headset_status();//sayanta ...not present in 32 kernel as of now
		//commenting as Headset status checking function is not proper in SLSI platform
...
void wm8994_set_playback_headset(struct snd_soc_codec *codec)
{
	...
	//msleep(20);	// 5ms delay//commenting sleep as not required in SLSI platform
	...
	//msleep(160);	// 160ms delay//commenting sleep as not required in SLSI platform
...
void wm8994_set_voicecall_common_setting(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, WM8994_CLOCKING_1,
		WM8994_DSP_FS2CLK_ENA | WM8994_DSP_FSINTCLK_ENA | WM8994_SYSCLK_SRC);
