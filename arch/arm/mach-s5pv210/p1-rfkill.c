...
//	enable_irq(IRQ_BT_HOST_WAKE);
	...
	//bt_rfk->name = bt_name;
	//bt_rfk->state = RFKILL_USER_STATE_SOFT_BLOCKED;
...
static int __init jupiter_btsleep_probe(struct platform_device *pdev)
{
	...
	//bt_sleep->state = RFKILL_USER_STATE_UNBLOCKED;
	/* userspace cannot take exclusive control */
	...
	//printk("[BT] rfkill_force_state(bt_sleep, RFKILL_USER_STATE_UNBLOCKED) \n");
	//rfkill_force_state(bt_sleep, RFKILL_USER_STATE_UNBLOCKED);
