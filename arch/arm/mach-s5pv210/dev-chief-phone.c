...
static void onedram_cfg_gpio(void)
{
	...
	set_irq_type(gpio_to_irq(GPIO_nINT_ONEDRAM_AP), IRQ_TYPE_LEVEL_LOW);
}
...
#elif defined(CONFIG_STEALTHV_USA)|| defined(CONFIG_MACH_AEGIS) || defined(CONFIG_MACH_VIPER) || defined(CONFIG_MACH_CHIEF)
static struct modemctl_platform_data mdmctl_data = {
	.name = "lte",
...
static void modemctl_cfg_gpio(void)
{
	...
	set_irq_type(gpio_to_irq(gpio_phone_active), IRQ_TYPE_EDGE_BOTH);
/*
	s3c_gpio_cfgpin(gpio_sim_ndetect, S3C_GPIO_SFN(0xF));
	s3c_gpio_setpull(gpio_sim_ndetect, S3C_GPIO_PULL_NONE);
	set_irq_type(gpio_to_irq(gpio_sim_ndetect), IRQ_TYPE_EDGE_BOTH);
*/
}
