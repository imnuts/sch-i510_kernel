...
static int bluetooth_set_power(void *data, enum rfkill_user_states state)
{
	...
	case RFKILL_USER_STATE_UNBLOCKED:
		...
		if (gpio_is_valid(GPIO_BTnRST))
			gpio_direction_output(GPIO_BTnRST, GPIO_LEVEL_LOW);

		pr_debug("[BT] GPIO_BT_nRST = %d\n",
				gpio_get_value(GPIO_BTnRST));
		...
		s3c_gpio_setpull(GPIO_BTnRST, S3C_GPIO_PULL_NONE);
		gpio_set_value(GPIO_BTnRST, GPIO_LEVEL_HIGH);

		s3c_gpio_slp_cfgpin(GPIO_BTnRST, S3C_GPIO_SLP_OUT1);
		s3c_gpio_slp_setpull_updown(GPIO_BTnRST, S3C_GPIO_PULL_NONE);
	...
	case RFKILL_USER_STATE_SOFT_BLOCKED:
		...
		s3c_gpio_setpull(GPIO_BTnRST, S3C_GPIO_PULL_NONE);
		gpio_set_value(GPIO_BTnRST, GPIO_LEVEL_LOW);

		s3c_gpio_slp_cfgpin(GPIO_BTnRST, S3C_GPIO_SLP_OUT0);
		s3c_gpio_slp_setpull_updown(GPIO_BTnRST, S3C_GPIO_PULL_NONE);
