...
//#define SEND_KEY_CHECK_TIME_MS	100		/* 50ms */
...
static void sec_jack_set_type(struct sec_jack_info *hi, int jack_type)
{
	...
	pdata->set_micbias_state(hi->send_key_irq_enabled);
    	...
    	if(hi->send_key_irq_enabled)
		pr_info("EAR_MICBIAS On\n");
    	else
		pr_info("EAR_MICBIAS Off\n");
...
static DEVICE_ATTR(select_jack, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH,
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		pr_err("[JACK] Failed to register switch device\n");
		goto err_switch_dev_register;
	}
