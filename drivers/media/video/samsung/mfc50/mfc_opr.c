...
static MFC_ERROR_CODE mfc_encode_one_frame(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	if (mfc_ctx->dynamic_bitrate != 0) {
		...
		WRITEL_SHARED_MEM((1 << 1) | (1 << 0), mfc_ctx->shared_mem_vaddr + 0x2c); /* framerate and GOP */
		WRITEL_SHARED_MEM(mfc_ctx->dynamic_framerate,  mfc_ctx->shared_mem_vaddr + 0x94);  /* framerate */
                WRITEL_SHARED_MEM(mfc_ctx->dynamic_framerate,  mfc_ctx->shared_mem_vaddr + 0x98);  /* GOP, IDR Peroid */
        } 
