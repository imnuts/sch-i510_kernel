...
#define IRQ_DPRAM_INT_N		IRQ_MSM
...
static int dpram_write(dpram_device_t *device, const unsigned char *buf, int len)
{
	...
	/* Internal DPRAM, check dpram ready?*/
	if (idpram_get_write_lock()) {
		printk(KERN_INFO "dpram_write - not ready return -EAGAIN\n");
		return -EAGAIN;
	}
...
static int dpram_read_fmt(dpram_device_t *device, const u16 non_cmd)
{
	...
	printk(KERN_DEBUG "idpram %s,  head: %d, tail: %d\n", __func__, head, tail);
...
static int dpram_read_raw(dpram_device_t *device, const u16 non_cmd)
{
	...
	//printk(KERN_ERR "idpram %s,  head: %d, tail: %d\n", __func__, head, tail);
...
static void command_handler(u16 cmd)
{
	...
	printk(KERN_DEBUG "[DPRAM] command_handler(0x%04X)\n", cmd);
	switch (cmd & 0xFF) {
...
//	printk(KERN_ERR "received mailboxAB = 0x%x@%xn", irq_mask,
...
{
	printk(KERN_DEBUG "%s - AP_CP_INT(%d), PDA_ACTIVE(%d)\n", __func__,
		gpio_get_value(GPIO_DPRAM_INT_N),
...
static int register_interrupt_handler(void)
{
	...
	if (err) {
		DPRAM_LOG_ERR("Host Wakeup Handler fail.\n");
