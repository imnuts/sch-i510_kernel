...
#define RNDIS_COMPOSITE  //Tethering mode : Rndis + DM + DM2 10.22 NN
//#define RNDIS_ADB // Rndis + adb for specific engeneer 
#define EP_INT_REALLOC 
#define DEBUG_OFF_UMS //when debugging off, set ums only  10.11.05
...
//#define ACM_DM_NMEA_CDROM_ADB_PRODUCT_ID 	0x0007	/* TEST PID !! Will be changed */
#define ACM_DM_NMEA_CDROM_ADB_PRODUCT_ID	0x68C3

#ifdef RNDIS_COMPOSITE
//#define RNDIS_DIAG_NMEA_PRODUCT_ID 	0x0008	/* TEST PID */
#define RNDIS_DIAG_NMEA_PRODUCT_ID	0x68C4
#endif
...
static int acm_dm_nmea_cdrom_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct =	__constant_cpu_to_le16(ACM_DM_NMEA_CDROM_ADB_PRODUCT_ID);
...
#ifdef RNDIS_COMPOSITE
static int rndis_diag_nmea_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct = __constant_cpu_to_le16(RNDIS_DIAG_NMEA_PRODUCT_ID);
	dev->cdev->desc.bDeviceClass	  = USB_CLASS_COMM;
	dev->cdev->desc.bDeviceSubClass   = 0x00;
	dev->cdev->desc.bDeviceProtocol   = 0x00;
...
#define	ANDROID_DEBUG_CONFIG_STRING "ACM + DM + NMEA + CDROM  + ADB (Debugging mode)"
...
#ifdef RNDIS_COMPOSITE
#define	ANDROID_DEBUG_RNDIS_CONIFG_STRING "RNDIS + DM + NMEA + ADB(debugging mode)"
#else
...
static struct usb_configuration acm_dm_nmea_cdrom_adb_config  = {
	.label		= ANDROID_DEBUG_CONFIG_STRING,
	.bind		= acm_dm_nmea_cdrom_adb_bind_config,
...
static struct usb_configuration rndis_only_config  = {
	.label		= ANDROID_DEBUG_RNDIS_CONIFG_STRING,
	.bind		= rndis_diag_nmea_bind_config,
...
recheck:
	...
	currentusbstatus=enable;
...
#else
	#ifdef RNDIS_COMPOSITE
		//make delay when ADB-> Tethering 
		if (oldusbstatus == USBSTATUS_ADB ||oldusbstatus == USBSTATUS_MTPONLY)	
			mdelay(1000);
	#endif 
		...
		else if(enable == USBSTATUS_UMS)
		{
			...
			UmsCDEnable=0;
#ifdef DEBUG_OFF_UMS
			printk("[enable_adb]  Debug OFF !!Skip recheck . Set UMS Only \n");
#else
			#ifdef EP_INT_REALLOC
			if(prev_enable_status == USBSTATUS_ADB && prev_status_before_adb != USBSTATUS_UMS &&prev_status_before_adb != USBSTATUS_VTP)
			#else
			...
			printk("[USB] %s -skip recheck ! set UMS Only !! prev_status(0x%02x), prev_status_before_adb setting(0x%02x)\n",
									__func__, prev_enable_status, prev_status_before_adb);
...
#ifdef RNDIS_COMPOSITE
			//make delay when Tethering ->ADB
			if (oldusbstatus == USBSTATUS_VTP)
				mdelay(1000);
#endif 
			ret = usb_change_config(dev->cdev, &acm_dm_nmea_cdrom_adb_config);
			if (ret) {
				printk("[%s] Fail to acm_dm_nmea_cdrom_adb_config()\n", __func__);
			}
			dev->adb_enabled = enable;
		}
...
*/
	...
	if (dev->cdev ==NULL || dev->cdev->gadget==NULL) 
	{
		return 0;
	}
}
...
extern void UsbMenuSelStore(int sel);
...
static int adb_enable_open(struct inode *ip, struct file *fp)
{
	...
	/* update ls->usb_func value */
	UsbMenuSelStore(USBSTATUS_ADB);
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	printk("[android_probe] pdata 0x%x\n", (unsigned int)pdata);	
	...
	if (pdata) {
		...
		printk("[android_probe] pdata->nluns %d \n", pdata->nluns);
