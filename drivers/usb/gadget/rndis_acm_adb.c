...
#define RNDIS_ACM_ADB_PRODUCT_ID 	0x0003	/* TEST PID */
#define RNDIS_ACM_DIAG_ADB_PRODUCT_ID 	0x0005	/* TEST PID */
#define RNDIS_ACM_DIAG_NMEA_ADB_PRODUCT_ID 	0x0004	/* TEST PID */
#define RNDIS_ACM_DIAG_NMEA_CDROM_ADB_PRODUCT_ID 	0x0004	/* TEST PID */
...
static int __init android_bind_config(struct usb_configuration *c)
{
	...
	ret = rndis_bind_config(c, hostaddr);  //Interface 0,1
	if (ret) {
		printk("[%s] Fail to rndis_bind_config()\n", __func__);
		return ret;
	}

	ret = acm_function_add(c); //Interface 2, 3
	if (ret) {
...
#ifdef DIAG
	ret = diag_function_add(c); //Interface 4
	if (ret) {
		printk("[%s] Fail to diag_function_add()\n", __func__);
		return ret;
	}
#endif

#ifdef NMEA
	ret = nmea_function_add(c); //Interface 5
	if (ret) {
		printk("[%s] Fail to nmea_function_add()\n", __func__);
		return ret;
	}
#endif

#ifdef MASS
	ret = mass_storage_function_add(c, dev->nluns); //Interface 6
	if (ret) {
	...
	ret = adb_function_add(c); //Interface 7
	if (ret) {
...
static int rndis_acm_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct =	__constant_cpu_to_le16(RNDIS_ACM_ADB_PRODUCT_ID);
...
//Add DIAG 08.24 NN +
static int rndis_acm_diag_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct =	__constant_cpu_to_le16(RNDIS_ACM_DIAG_ADB_PRODUCT_ID);
	dev->cdev->desc.bDeviceClass	  = USB_CLASS_COMM;
	dev->cdev->desc.bDeviceSubClass   = 0x00;
	dev->cdev->desc.bDeviceProtocol   = 0x00;
...
#ifdef DIAG	
	ret = diag_function_config_changed(dev->cdev, c);
	if (ret) {
		printk("[%s] Fail to diag_function_config_changed()\n", __func__);
		return ret;
	}
#endif
...
static int rndis_acm_diag_nmea_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct =	__constant_cpu_to_le16(RNDIS_ACM_DIAG_NMEA_ADB_PRODUCT_ID);
	dev->cdev->desc.bDeviceClass	  = USB_CLASS_COMM;
	dev->cdev->desc.bDeviceSubClass   = 0x00;
	dev->cdev->desc.bDeviceProtocol   = 0x00;
...
#ifdef NMEA
		ret = nmea_function_config_changed(dev->cdev, c);
		if (ret) {
			printk("[%s] Fail to nmea_function_config_changed()\n", __func__);
			return ret;
		}
#endif
...
static int rndis_acm_diag_nmea_cdrom_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;

	dev->cdev->desc.idProduct =	__constant_cpu_to_le16(RNDIS_ACM_DIAG_NMEA_CDROM_ADB_PRODUCT_ID);
	dev->cdev->desc.bDeviceClass	  = USB_CLASS_COMM;
	dev->cdev->desc.bDeviceSubClass   = 0x00;
	dev->cdev->desc.bDeviceProtocol   = 0x00;
...
#define	ANDROID_DEBUG_TEST_CONFIG_STRING "RNDIS + ACM + ADB (Debugging mode)"
//Add DIAG 08.19 NN
#define	ANDROID_DEBUG_DIAG_TEST_CONFIG_STRING "RNDIS + ACM + DIAG + ADB (Debugging mode)"
//Add Nmea
#define	ANDROID_DEBUG_NMEA_TEST_CONFIG_STRING "RNDIS + ACM + DIAG + NMEA+ ADB (Debugging mode)"
#define	ANDROID_DEBUG_NMEA_CDROM_TEST_CONFIG_STRING "RNDIS + ACM + DIAG + NMEA+ CDROM+ ADB (Debugging mode)"
...
static struct usb_configuration rndis_acm_adb_config  = {
	.label		= ANDROID_DEBUG_TEST_CONFIG_STRING,
	.bind		= rndis_acm_adb_bind_config,
	.bConfigurationValue = 3,
	.bmAttributes	= USB_CONFIG_ATT_ONE | USB_CONFIG_ATT_SELFPOWER,
	.bMaxPower	= 0x30, /*  x2 = 160ma */
};
//Add DIAG 08.19 NN
static struct usb_configuration rndis_acm_diag_adb_config  = {
	.label		= ANDROID_DEBUG_DIAG_TEST_CONFIG_STRING,
	.bind		= rndis_acm_diag_adb_bind_config,
	.bConfigurationValue = 4,
	.bmAttributes	= USB_CONFIG_ATT_ONE | USB_CONFIG_ATT_SELFPOWER,
	.bMaxPower	= 0x30, /*  x2 = 160ma */
};
//Add NMEA
static struct usb_configuration rndis_acm_diag_nmea_adb_config  = {
	.label		= ANDROID_DEBUG_NMEA_TEST_CONFIG_STRING,
	.bind		= rndis_acm_diag_nmea_adb_bind_config,
	.bConfigurationValue = 5,
	.bmAttributes	= USB_CONFIG_ATT_ONE | USB_CONFIG_ATT_SELFPOWER,
	.bMaxPower	= 0x30, /*  x2 = 160ma */
};
//Add CDROM
static struct usb_configuration rndis_acm_diag_nmea_cdrom_adb_config  = {
	.label		= ANDROID_DEBUG_NMEA_CDROM_TEST_CONFIG_STRING,
	.bind		= rndis_acm_diag_nmea_cdrom_adb_bind_config,
...
// Fix: USBCV Ch9 RemoteWakeup Enable/Disable Test.
	...
	ret = usb_change_config(dev->cdev, &rndis_acm_diag_nmea_cdrom_adb_config); 
	#else
	ret = usb_change_config(dev->cdev, &rndis_acm_diag_nmea_adb_config);	
	#endif
	if (ret) {
...
#endif	
		printk("[enable_adb] rndis_acm_diag_nmea_cdrom_adb_config\n");
...
static void enable_askon(struct android_dev *dev, int enable)
{
	...
	printk("[enable_askon]  rndis_acm_diag_nmea_cdrom_adb_config \n");
	mtp_mode_on = 0;
	inaskonstatus = 1;			
	UmsCDEnable=0;
	UsbIndicator(1);
	...
	ret = usb_change_config(dev->cdev, &rndis_acm_diag_nmea_cdrom_adb_config);
	if (ret) {
		printk("[%s] Fail to rndis_acm_diag_nmea_cdrom_adb_config \n", __func__);
	}
