...
static int  vnet_recv_rx_data(u8 *buf, int size, struct pdp_info *dev)
{
	...
	memcpy((void *)skb->data, (void *)buf, size);
		...
		case HN_PDP_ACTIVATE:
			{
				...
				if (g_radio == LTE) {
					MULTIPDP_LOG_INFO ("HN_PDP_ACTIVATE calling pdp_activate with ONEDRAM_DEV \n");
...
static int multipdp_init(void)
{
	...
	ret = pdp_activate(&ptp_arg, DEV_TYPE_SERIAL, DPRAM_DEV, DEV_FLAG_STICKY);
	if (ret < 0) {
...
void register_multipdp_debug_control_attribute(void)
{
	return device_create_file(multipdp_dev.this_device, &dev_attr_multipdp_debug);
