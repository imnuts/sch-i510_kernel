...
extern void FSA9480_ShowRegisterStatus(void);
...
static int chg_first_call = 1;	// hanapark_DI02

extern spinlock_t pmic_access_lock;
extern unsigned int HWREV;
extern struct input_dev *g_dev;
...
void set_low_bat_interrupt_mask_enable(int on)
{
	if(on)
	{
		Set_MAX8998_PM_REG(LOBAT2M, 1);
		Set_MAX8998_PM_REG(LOBAT1M, 1);
	}
	else
	{	
		Set_MAX8998_PM_REG(LOBAT2M, 0);
		Set_MAX8998_PM_REG(LOBAT1M, 0);
	}
}
...
void MAX8998_IRQ_init(void)
{
	...
	irq_mask[0] = (HWREV == 8) ? (byte)IRQ1_M_PWR : (byte)IRQ1_M;
	...
	Set_MAX8998_PM_REG(LBTH1,0x5);	// 3.4V
...
static int wc_detected = 0;	// hanapark (wireless charging)
...
void maxim_vac_connect(void)
{
	...
	if(Get_MAX8998_PM_REG(VDCINOK_status))
	{
		if(curent_device_type == PM_CHARGER_NULL)
		{
			...
			if(UsbStatus){
				pr_info("%s: maxim_USB_connect~~~\n", __func__);
				curent_device_type = PM_CHARGER_USB_INSERT;
			}
			else{
				pr_info("%s: maxim_TA_connect~~~\n", __func__);
            ...
            /* add debug code for FSA9480 */
            FSA9480_ShowRegisterStatus();
		}
	}
	else
	{
...
void maxim_pwr_press(void)
{
	if(HWREV == 8) {
		if(g_dev) {
			input_report_key(g_dev, 26, 1);
			printk("%s: pwr key pressed\n", __func__);
		} else
			printk("%s: pwr key device not initialize\n", __func__);
	}
}
void maxim_pwr_release(void)
{
	if(HWREV == 8) {
		if(g_dev) {
			input_report_key(g_dev, 26, 0);
			printk("%s, pwr key released\n", __func__);
		} else
			printk("%s: pwr key device not initialize\n", __func__);
	}
}
void maxim_low_battery_2nd(void)
{
	printk("%s\n",__func__);
...
void maxim_low_battery_1st(void)
{
	printk("%s\n",__func__);
...
unsigned char maxim_chg_status(void)
{
	...
	if(Get_MAX8998_PM_REG(VDCINOK_status)){
		...
		if(UsbStatus){
			printk("%s: maxim_USB_connect~~~ \n", __func__);
			curent_device_type = PM_CHARGER_USB_INSERT;
		}
		else{
			printk("%s: maxim_TA_connect~~~ \n", __func__);
			curent_device_type = PM_CHARGER_TA;
			MicroTAstatus = 1;
		}
		return 1;
...
void maxim_charging_control(unsigned int dev_type, unsigned int cmd)
{
	...
	if (!cmd) {
		...
		pr_info("%s: charging disable \n",__func__);
	} else if (dev_type == PM_CHARGER_TA) {
#ifdef CONFIG_WIRELESS_CHARGING
		if (gpio_get_value(GPIO_WC_DETECT) == 0) {
				/* Wireless charging */
			reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x2<<0); // CHG_TOPOFF_TH=10%, CHG_RST_HYS=Disable, AC_FCGH=475mA
			...
			pr_info("%s: WIRELESS charging enable \n", __func__);
 		} else
#endif	/* CONFIG_WIRELESS_CHARGING */
		{
			/* TA charging */
			reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x7<<0); // CHG_TOPOFF_TH=10%, CHG_RST_HYS=Disable, AC_FCGH=800mA
			reg_buff[1] = (0x2<<6) | (0x3<<4) | (0x0<<3) | (0x0<<1) | (0x0<<0); //ESAFEOUT1,2= 10, FCHG_TMR=disable, MBAT_REG_TH=4.2V, MBATT_THERM_REG=105C
			Set_MAX8998_PM_ADDR(CHGR1, reg_buff, 2); 
			pr_info("%s: TA charging enable \n", __func__);
		}		
	} else if (dev_type == PM_CHARGER_USB_INSERT) {
		reg_buff[0] = (0x0<<5) | (0x3<<3) | (0x2<<0) ; // CHG_TOPOFF_TH=10%, CHG_RST_HYS=Disable, AC_FCGH=475mA
		if (FSA9480_PMIC_CP_USB()) {
			if (askonstatus)
