...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.atchcalsthr = 30; //35;
...
void qt_Multitouchscreen_Init(void)
{
    ...
    touchscreen_config.xedgedist = 50; //40;
    touchscreen_config.yedgectrl = 143;
    touchscreen_config.yedgedist = 90; //80;
...
void qt_Noise_Suppression_Config_Init(void)
{
    ...
    noise_suppression_config.freq[0] = 29;
    noise_suppression_config.freq[1] = 34;
    noise_suppression_config.freq[2] = 39;
    noise_suppression_config.freq[3] = 49;
    noise_suppression_config.freq[4] = 58;
...
//			QT_printf("[TSP] Report ID : %d, Object Type : T%d, Instance : %d\n\r",report_id ,object_type,instance);
	...
	read_dbg_data(QT_REFERENCE_MODE, 1, &qt_refrence);
	printk("\n[TSP] current reference : %d\n", qt_refrence);

    if( qt_refrence > 13500 ) {
...
get_ta_status(void) {
	int wc_detected = 0;
#ifdef CONFIG_WIRELESS_CHARGING
	if ((HWREV > 8) && (gpio_get_value(GPIO_WC_DETECT) == 0))
		wc_detected = 1;
#endif

    if( Get_MAX8998_PM_REG(VDCINOK_status) && (wc_detected == 0))
        return 1;
    else
        return 0;
}
...
#ifdef QT_FIRMUP_ENABLE
{
    ...
    /* check TA status */
    if( get_ta_status() ) {
        printk(KERN_DEBUG "qt602240_probe: TA is connected -> update TSP register\n");
