...
#define YAS_ERROR_TRANSFER                  (-9)
#define YAS_ERROR_INTERRUPT                 (-10)
...
//#else
    int (*read_reg_yas530)(uint8_t addr, uint8_t *buf, int len);
    int (*write_reg_yas530)(uint8_t addr, const uint8_t *buf, int len);
//#endif
...
struct yas_gyro_driver_callback {
    ...
    int (*device_open)(void);
    int (*device_close)(void);
    int (*device_write)(uint8_t adr, uint8_t *buf, int len);
    int (*device_read)(uint8_t adr, uint8_t *buf, int len);
