...
atomic_t factorytest_proxoff; 
static char prox_value=1;
...
static void gp2a_work_func_prox(struct work_struct *work)
{
	...
	vout = value & 0x01;
	printk(KERN_INFO "[PROXIMITY] value = %d  int_val=%d\n",vout,int_val);
...
irqreturn_t gp2a_irq_handler(int irq, void *dev_id)
{
...
#ifdef PROX_MODE_A   
    value = gpio_get_value(GPIO_PS_VOUT);
    printk("MODE-A [PROXIMITY] IRQ_HANDLED %d \n", value);
#endif
    ...
    gprintk("\n");
#ifdef PROX_MODE_B 
    schedule_work(&prox_data->work_prox);
#endif
    return IRQ_HANDLED;
}
#ifdef PROX_MODE_B
static void gp2a_work_func_prox(struct work_struct *work)
{
   ...
   // Read the VO bit value from 00 register 
    ...
    printk("Mode-B1 Proximity Value=%d prox_value =%d\n",value,prox_value);
}
#endif
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	else if(value==3 && proximity_enable == OFF)
	{
        ...
        atomic_set(&factorytest_proxoff, OFF);
	...
	else if(value==2 && proximity_enable == ON)
	{
        ...
        atomic_set(&factorytest_proxoff, ON);
		printk("[PROXIMITY] Temporary : Power OFF\n");

	}
	/* for factory simple test mode */
	else if(value==7 && light_enable == OFF)
	{
	//	light_init_period = 2;
        printk(KERN_INFO "Factory Light enable for mdnie testing\n"); 
		gp2a_on(gp2a,LIGHT);
		value = 7;
	}
    else if( 8 == value && ON == light_enable)
    {
       printk(KERN_INFO "Factory Light disable for mdnie testing\n");
       gp2a_off(gp2a,LIGHT);
       value =8;
...
//	ret = request_irq(irq, gp2a_irq_handler, IRQF_DISABLED, "proximity_int", gp2a);
    ret = request_irq(irq, gp2a_irq_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "proximity_int", gp2a);
...
static int proximity_open(struct inode *ip, struct file *fp)
{
    ...
    if(!(atomic_read(&factorytest_proxoff)))
      proximity_onoff(1);
...
static ssize_t proximity_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	//printk("[%s] \n",__func__);
#ifdef PROX_MODE_A
    prox_value = gpio_get_value(GPIO_PS_VOUT);
#endif
	//printk("[%s]  %d\n",__func__, prox_value);
    put_user(prox_value, buf);
	return 1;
}
