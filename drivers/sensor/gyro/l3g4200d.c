...
static ssize_t l3g4200d_get_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;
	int res;
	unsigned char gyro_data[6];
	...
	printk("[l3g4200d_get_data] temp = 0x%x, bZYXDA = %d\n", temp[0], bZYXDA);
#endif
...
#if DEBUG
	printk("[l3g4200d_get_data] raw[0] = %d\n", raw[0]);
	printk("[l3g4200d_get_data] raw[1] = %d\n", raw[1]);
	printk("[l3g4200d_get_data] raw[2] = %d\n", raw[2]);
	printk("\n");	
#endif
#if 0
	// change to dps
	raw[0] = raw[0] * 875 / 100000;
	raw[1] = raw[1] * 875 / 100000;
	raw[2] = raw[2] * 875 / 100000;
#endif
...
static DEVICE_ATTR(gyro_get_data, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, l3g4200d_get_data, NULL);
			       ...
			       const struct i2c_device_id *devid)
{
	...
	if ( (err = device_create_file(dev, &dev_attr_gyro_get_data)) < 0)
	{
		printk("Failed to create device file(%s)!\n", dev_attr_gyro_get_data.attr.name);
		goto error_destroy;
	}	
	
	printk(KERN_INFO "%s : L3G4200D device created and probe completed successfully\n", __func__);
